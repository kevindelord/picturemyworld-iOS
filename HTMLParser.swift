...
private extension HTMLParser {
	...
	private static func generateImageURL(fromThumbnailURL url: String) -> String? {
		...
		return url.stringByReplacingOccurrencesOfString("thumb", withString: "large")
	}
	...
	private static func generateIdentifier(fromThumbnailURL url: String) -> String? {
		...
		guard let filename = HTMLParser.matchesForRegexInText("[0-9A-Za-z_-]+.[a-z]{3}$", text: url).first else {
			return nil
		}
		let identifier = HTMLParser.matchesForRegexInText("^[0-9A-Za-z_-]+", text: filename).first
		return identifier
	}
